...
> 以上 18 个错误，本质上归结为以下 6 个核心知识点。理解了这些，同类错误不会再犯。
...
```c
char host[64];
host = optarg;              // 错误1：数组名是常量，不能赋值

get_time(&(data.time), 64); // 错误2：&多了，类型变成"指向数组的指针"
```

**核心概念**：

```
char host[64];    → 在内存中占据 64 字节的连续空间
host              → 值是这段空间的起始地址，但这个值是"编译期常量"
                   你不能改它，就像你不能改"你家门牌号"

optarg            → 是一个 char* 指针，指向另一个内存区域
host = optarg     → 你想把"门牌号"改成别人的门牌号？不行。

strncpy(host, optarg, 63)  → 把 optarg 指向的内容"搬"到 host 的空间里 ✓
```

**数组传参时的退化规则**：
```
data.time        → 自动退化为 char*     (指向首字符)       ✓
&(data.time)     → 类型是 char(*)[64]   (指向整个数组)     ✗ 类型不匹配
```

**一句话记住**：数组是"容器"，指针是"路标"。容器不能被赋值，只能往里装东西。

---

### 知识点 2：结构体的 `.` 和 `->` 取决于变量本身是不是指针

```c
socket_t sock;       // 栈上的结构体变量
socket_t *p = &sock; // 指向它的指针
...
**底层原理**：
```
sock.fd   编译器翻译为:  *(地址 + fd的偏移量)
p->fd    编译器翻译为:  *(*(p) + fd的偏移量)
```

**一句话记住**：看声明时有没有 `*`。有 `*` 用 `->`，没 `*` 用 `.`。
...
```c
continue;    // 只能在 for/while/do-while 循环体内
break;       // 只能在循环或 switch 内
return;      // 只能在函数体内
```

你的错误是在循环外面用了 `continue`。编译器会直接报错。

**一句话记住**：`continue` 和 `break` 是"循环的遥控器"，在循环外面按遥控器没反应。

---

### 知识点 4：`sizeof` 返回字节数，不是元素个数

```c
struct epoll_event events[1024];

sizeof(events)                    = 12 × 1024 = 12288  (字节数)
sizeof(events) / sizeof(events[0]) = 1024               (元素个数)
```

你把 12288 传给了 `epoll_wait`，它会尝试读 12288 个事件，越界访问内存。

**一句话记住**：`sizeof` 是"字节尺子"，要数个数得除以单个的大小。

---

### 知识点 5：资源所有权 — 谁打开谁关闭

这是你项目中最严重的错误类别，涉及 3 个问题：

```c
// 问题A：函数关闭了不属于自己的资源
sqlite3_stmt* data_exist(sqlite3 *db) {
    if(fail) {
        sqlite3_close(db);  // ← db不是我打开的，我不该关它
    }
}

// 问题B：while(1)后面的清理代码永远执行不到
while(1) { ... }
close(sock.fd);      // ← 死代码
sqlite3_close(db);   // ← 死代码

// 问题C：函数失败了没检查，继续用无效资源
temporary_repo(&db);  // 失败了db可能无效
// ... 继续用db操作 → 崩溃
```
...
谁申请的资源        谁负责释放         典型错误
─────────────────────────────────────────────────
main中的db          main关闭          被子函数关闭了
main中的sock.fd     main关闭          while(1)后面关不到
malloc的内存        同一函数free       忘了free或重复free
fopen的FILE*        同一函数fclose     错误路径忘了fclose
getaddrinfo的result 同一函数freeaddrinfo  循环break后忘了free
```

**你需要养成的思维习惯**：
```
写到 open/malloc/socket/sqlite3_open 时
    → 立刻在下面写对应的 close/free
    → 然后检查每个错误分支是否也能执行到释放代码
```

**一句话记住**：谁借的谁还，借了还不起（while(1)出不去）也要想办法还。

---

### 知识点 6：头文件是"合同"，声明和实现必须一致

```c
// packet.h 声明了：我接受 4 个参数
int date_packet(char *time, double *temperature, char *buf, size_t buf_len);

// packet.c 实现了：我只收 3 个参数
int date_packet(data_t *data, char *buf, size_t buf_len);
```

调用者按合同传 4 个参数，实现只收 3 个 → 第 4 个参数残留在栈上 → 栈被破坏。

**头文件的三个常见坑**：

| 坑 | 你犯的错 | 后果 |
|---|---|---|
| `#include` 在 `#ifndef` 外 | packet.h | 重复包含可能重定义 |
| 缺少 `#include` | database.h 没有 sqlite3.h | 类型未定义 |
| 声明和实现不一致 | packet.h vs packet.c | 栈破坏 |

**一句话记住**：头文件是"合同"，实现是"交付"。合同和交付不一致就是违约（崩溃）。
...
类别           数量    根本原因
──────────────────────────────────────
基础语法错误    4     对数组/指针/控制流理解不扎实
变量拼写错误    2     手写代码不检查，缺少IDE辅助
头文件错误      3     不理解头文件的"合同"作用
函数签名不匹配  1     改了实现没更新声明
逻辑/运行时     3     sizeof含义混淆，死循环出口没设计好
类型安全        1     信号处理的特殊要求不了解
资源泄漏        3     没有建立"所有权"意识
环境适配        1     硬编码路径，没有可移植性思维
```

**最大的改进空间**：资源管理（错误 16-18）。这不只是语法问题，是**架构思维**。每次打开一个资源，都要想清楚"谁负责关闭它"、"所有退出路径都能关闭吗"。
